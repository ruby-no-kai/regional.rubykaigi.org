...
class RegionalRubyKaigiCliTest < Minitest::Test
  def test_validate_kaigis_reports_success_for_valid_data
    fixture_dir = File.expand_path("../fixtures/data", __dir__)
    out = StringIO.new
    err = StringIO.new

    status = RegionalRubyKaigi::CLI.validate_kaigis(data_dir: fixture_dir, out: out, err: err)
  ...
  def test_validate_kaigis_reports_failure_for_invalid_data
    Dir.mktmpdir do |dir|
      ...
      status = RegionalRubyKaigi::CLI.validate_kaigis(data_dir: dir, out: out, err: err)
